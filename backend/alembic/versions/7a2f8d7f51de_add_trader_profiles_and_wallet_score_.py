...
down_revision: Union[str, Sequence[str], None] = '092c9ceebb96'
...
def upgrade() -> None:
    ...
    op.execute("CREATE INDEX IF NOT EXISTS ix_tp_primary_category ON trader_profiles (primary_category)")

    # New columns on wallet_scores
    op.add_column('wallet_scores', sa.Column('roi', sa.Float(), nullable=False, server_default='0'))
    op.add_column('wallet_scores', sa.Column('consistency', sa.Float(), nullable=False, server_default='0'))
